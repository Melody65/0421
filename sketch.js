...
function setup() {
  ...
  button4.mousePressed(() => {
    ...
    // 創建一個模擬視窗的對話框
    let videoDialog = createDiv();
    videoDialog.style('position', 'absolute');
    videoDialog.style('width', '600px'); // 統一視窗寬度
    videoDialog.style('height', '400px'); // 統一視窗高度
    videoDialog.style('background-color', '#ffffff');
    videoDialog.style('border', '2px solid #000000');
    videoDialog.style('border-radius', '10px');
    videoDialog.style('box-shadow', '0 4px 8px rgba(0, 0, 0, 0.2)');
    videoDialog.style('padding', '20px');
    videoDialog.style('text-align', 'center');
    videoDialog.position((windowWidth - 600) / 2, (windowHeight - 400) / 2); // 居中顯示

    // 添加影片元素
    let video = createVideo(['教學影片.mp4']);
    video.size(560, 315); // 設置影片大小
    video.style('margin-top', '20px');
    video.parent(videoDialog);
    video.attribute('controls', ''); // 添加播放控制
    ...
    closeButton.style('border-radius', '5px');
    closeButton.style('cursor', 'pointer');
    closeButton.parent(videoDialog);
    closeButton.mousePressed(() => {
      videoDialog.remove(); // 關閉視窗
    });
  });
